...
![[../../annex/串行Flash文件系统FatFs_image_81.png]]
标志位用或|来括起来，（给权限）

递归函数：递归就是一个函数在它的函数体内调用它自身。执行递归函数将反复调用其自身，每调用一次就进入新的一层。递归函数必须有结束条件。
每次递归调用的时候都会重新申请这些变量，所以说，如果递归深度很深的话，函数层层嵌套，他们申请的空间都是栈空间，有可能会溢出。所以说只要使用了 SCAN_FILE 或某些递归函数的话，一定要注意栈空间一定要分配大一点。否则很容易导致溢出。
![[../../annex/串行Flash文件系统FatFs_image_82.png]]
而且这些变量是没法定义到全局变量中的。定义在全局变量可能就没什么用了。
...
## P 68 如何恢复 FLASH 的出厂内容

把 srcdata 文件夹复制到 u 盘中（通过读卡器拷贝到 sd 卡中），然后弹出 SD 卡，接到开发板上。重新上电。再编译刷外部 FLASH 的程序。

![[../../annex/串行Flash文件系统FatFs_image_83.png]]

Main 文件分成非文件系统不分、文件系统部分
![[../../annex/串行Flash文件系统FatFs_image_84.png]]
前 2 M 字节的文件会直接存储在这些地址空间。某些程序可能会需要这些数据，那么就让他直接从这些地址开始读取这些数据进行使用。
![[../../annex/串行Flash文件系统FatFs_image_85.png]]
为什么分成非文件系统和文件系统？
直接用文件格式来读取并不好，因为如果用文件系统来读取的话，速度会非常慢。
...
如何使用刷新程序恢复 STM 32 F 103 的出厂数据包括使用 SD 卡拷贝数据、将数据拷贝到 SD 卡、将 SD 卡插入板子并运行刷新程序等步骤。视频中详细演示了刷新程序的操作过程并提醒注意不要在下载其他程序前断电。最后, 视频还介绍了刷新程序的原理, 以及如何根据需要修改程序来烧录新的内容到 flash 中。

- 如何刷新 spiflash, 恢复出厂数据并演示了刷新过程。同时还介绍了指南者的刷新方法。
00:07 介绍了如何通过烧写和读取数据来操作 SPFLASH，包括使用文件系统和直接操作 FLASH。
00:36 液晶显示演示: 通过液晶显示演示，说明了在演示过程中可能用到的 SPIFLASH 的数据。
02:27 介绍了如何通过复制 src data 文件到 U 盘或 SD 卡，然后烧录到板子上，来恢复出厂数据

- 一个程序的拷贝过程分为非文件系统和文件系统两部分, 并解释了为什么这样分。
05:50 文件系统和数据的复制过程
08:47 程序原理和烧录新内容的方法
11:27 数据读取和文件系统的优势

- 在复制文件时如何通过 SD 卡读取文件并直接存储在非文件系统部分的 SPI flash 中
11:44 运行文件系统慢，数据直接存储在 FLASH 中
15:38 结构体中包含要拷贝的文件信息
17:16 烧写选项: 验证、静默、签名和验证签名

- 烧录程序中的变量和信息结构, 以及烧录文件的选择和烧录过程的详细介绍。
17:35 烧录文件系统的过程
21:07 烧录和验证过程中的关键函数
22:03 使用 scan file 函数扫描文件夹并烧录文件

- 如何使用一个函数来将指定目录下的文件复制到另一个位置并详细讲解了具体实现过程。
22:24 拷贝文件和目录到 SD 卡
23:31 挂载和格式化文件系统
24:43 使用 copydir 函数复制目录和文件
