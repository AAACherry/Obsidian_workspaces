...
默认情况下，使用存储设备 1 个。现在用了两个存储设备（SD 和 Flash）。所以就会找不到这个设备，所以会报错：FR_INVALID_DRIVE (11)，将 ffconf.c 文件中的 VOLUMES 的值改为 2 即可。
...
所以我们可以通过把 ffconf.C 文件中的 MAX_SS 配置成跟我们的扇区大小一致，就不会出现这种报错情况（挂载失败）。
