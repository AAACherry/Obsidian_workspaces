...
RGB 888 -- 红绿蓝分量用 8 个数据位来表示组成的格式
常见的颜色表示会在“RGB”后面附带各个颜色分量值的<font color="#ff0000">数据位数</font>，如 RGB565表示红绿蓝的<font color="#ff0000">数据线数</font>分别为5、6、5根，一共为16个数据位，可表示216种颜色；而这个液晶屏的种颜色分量的数据线都有8根，所以它支持 RGB888格式，一共24位数据线，可表示的颜色为224种。
...
液晶屏中的每个像素点都是数据，在实际应用中需要把每个像素点的数据缓存起来，再传输给液晶屏，这种存储显示数据的存储器被称为显存。显存一般至少要能存储液晶屏的一帧显示数据，如分辨率为800x480 的液晶屏，使用 RGB888 格式显示，它的一帧显示数据大小为：3x800x480=1152000字节；若使用 RGB565格式显示，一帧显示数据大小为：2x800x480=768000字节。
 ...
 系数 3 和2 是根据每个像素所需字节数而来（8+8+8）÷8=3、（5+6+5）÷8=2
```
RGB888和RGB565是指像素数据的存储格式，表示红色、绿色和蓝色三种颜色通道的位数。

- RGB888格式意味着每个颜色通道（红、绿、蓝）都有8位，即每个通道可以表示$2^8=256$种颜色。因此，每个像素需要$8+8+8=24$位或3字节（因为1字节=8位）来存储。
- RGB565格式意味着红色通道有5位，绿色通道有6位，蓝色通道有5位，总共16位或2字节（因为1字节=8位）来存储一个像素的颜色信息。

所以，对于分辨率为800x480的液晶屏：

- 使用RGB888格式显示时，每个像素需要3字节，所以一帧显示数据大小为$3 \times 800 \times 480 = 1152000$字节。
- 使用RGB565格式显示时，每个像素需要2字节，所以一帧显示数据大小为$2 \times 800 \times 480 = 768000$字节。

这里的3和2分别是RGB888和RGB565格式下，每个像素需要的字节数。
```
