...
再说这个堆和栈这块呢扩展一下堆栈一起记忆, 先说这个站, 就是说这个函数被调用的时候, 会这个系统会开辟这个占空间, 而这个堆呢是由用户自己去那个买 lock, 一个用多少就买 lock 多少, 但是呢也要注意 free 去自己去回收, 这完全是手动的, 你那个忘记回收了, 可能不是可能一定会造成内存泄露, 但是站那就用户不用担心这个了, 你这个函数调完了之后, 系统自动的会把这个占空间进行一个回收, 这样呢可以反复的利用, 咱们呢再上第二个图, 这个图是不是看着更直观一些, 左边是这个 ram, 右边是 room, 用这个不同的颜色代表呢存储的是不同的数据, 先从 room 来看, 他有这个 test, 也就是说这个用户的函数, 这个 r o 那就是 read only date, 它是指那个全局变量（初值数据）, 还有这个 const const 是只读的这个变量, 而这个 start 是什么意思呢, 我们这个单片机在一上电的时候, 必然会有一个启动的首地址, 一般呢会从这个地址零零开始跑, 也就是 cpu 从地址零零开始跑, 或者呢是从用户设定的一个初始地址开始跑, 一般是零零, 咱们就这么记忆就得了, 就是从这个函数开始跑起, 这里看着两个粉色的, 左边右边是这个 room room date or date, 右边呢是这个 ram, 这边有个 date, 就是说上电的时候啊, 这个系统自然会把这个 i dta 的数据, 拷贝到这个 ram 的一个区域, 等到这个函数被调用的时候, 这个绿色的这个 stock 占空间开始进行堆栈操作, 这个寄存器啊, 是我们在这个初始化这个模块的时候, 初始化单片机的时候会写入一些参数, 会写入到这个计算器区, 当然了, 寄存器区的数据也相当于掉电丢失, 因为那寄存器是线上店现配的, 现生效的是这样的, 还有这个堆, 如果用户用用到了买 lock, 那必然这个要分配一个堆空间, 当然了, 这些都是用户自己去设定, 以上呢就是这个关于单片机内存课程的全部, 如果呢您有疑问或者呢浪人哪里讲的不清楚, 欢迎留言一起讨论, 关注不迷路, 更多干货在后面哟,
